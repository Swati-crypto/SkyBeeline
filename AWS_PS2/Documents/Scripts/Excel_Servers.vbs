...
'Check If File Is Already placed at correct location 
If (objFS.FileExists(strExecutionpath & "\" & Ucase(objSysName) & "\" & Ucase(strXlsFolderName) & "\" & strXlsFileName)) Then 
 xlsFlag=0
End If

If (objFS.FileExists(strExecutionpath & "\" & Ucase(objSysName) & "\" & Ucase(strXlsFolderName) & "\" & strCommonFileName)) Then 
 xlsCOMMONFlag=0
End If
...
'Create a Destination path inside the strExecutionpath >> Ntnet User >> Excel File Named Folder
strDestinationLocation = strExecutionpath & "\" & Ucase(objSysName) & "\" & Ucase(strXlsFolderName)
